...
def create_header():                # This function name is not descriptive enough, use the one below
  """
...
def range_to_num_bits(range_str):
  ...
  range = re.split('\[|\<', range_str)  # Get the range section at the end of the bus name
  if(len(range) > 1):
    range = range[1]
  else:
    return 1                            # It's a bare signal name without brackets (eg sig_name)

  range = range[:-1]                    # Remove the closing bracket 

  if(len(range.split(':')) == 1):       # There's brackets but no range, single bit (eg bus_name[6])
    return 1

  try:
    left = int(range.split(':')[0])            # Get the left bound (exception if not a number)
    right = int(range.split(':')[1])           # Get the right bound (exception if not a number)
    if(left >= right):
      return left - right + 1
    else:
      return right - left + 1
...
def to_hex_str(num, len=32):
  ...
  return str(hex(int(in_str, base))[2:].zfill(size/4))

def to_dec_str(num, len=32):
  """
  Converts <in_str> from it's source base which is automatically detected if not supplied in <in_base>.  
...
def list_instances(verilog_file_name):
  ...
  for line in verilog.split('\n'):
    ...
    if((_search_keyword(line, 'end')==True) & (_search_keyword(line, 'endcase') == False)):
      block_cnt = block_cnt - 1
    ...
    if(block_cnt >= 1):
      ...
      elif(sline[0] in ['always', 'initial', 'logic', 'wire', 'reg', 'input', 'output', 'inout', 'assign', 'integer']):
        pass
  ...
  for line in verilog.split('\n'):
    ...
    if(block_cnt >= 1):
      ...
      if(sline[0] in ['always', 'initial', 'logic', 'wire', 'reg', 'input', 'output', 'inout']):
        ...
        found_inst = True
      elif( (found_inst == True) &  (line.find('.') >= 0) & (line.find(')') > 0)):
        connect_list.append( tuple(''.join(c for c in line if c not  in ['.' , '(' , ')', ',']).split()) )        # Removes the .(), and returns the port connection.
